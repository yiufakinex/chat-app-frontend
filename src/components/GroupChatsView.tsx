            ...
            </div>
            {
                (groupChats.length === 0) ?
                    ...
                    groupChats.map(groupChat => {
                        return (
                            <div
                                className="border-bottom border-top text-decoration-none px-1"
                                key={groupChat.id}
                                onClick={() => setChat(groupChat)}
                                style={{ cursor: 'pointer', background: (groupChat.id === props.groupChatId && props.tab === 'chat') ? 'Gainsboro' : '' }}
                            >
